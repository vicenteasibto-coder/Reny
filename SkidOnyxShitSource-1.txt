...
-- Aetherea Advanced AC Bypass (combined)
pcall(function()
    ...
    if not ac_script or not ac_remote then warn("[Onyx]: AC objects not found, skipping advanced bypass") return end
            ...
            old_dbg_traceback = hookfunction(getrenv().debug.traceback, function(...)
                local ok, res = pcall(function(...)
                    ...
                    for Line in TracebackLines(Traceback, StackLevel) do
                        if not ValidTraceback(Line) then continue end
    ...
    local SafeHook = function(hookfn, ...)
        ...
        if not ok then warn("[Onyx]: SafeHook failed") end
    ...
    if not bypassed then warn("[Onyx]: AC bypass timed out, continuing anyway") end
    task.wait(1)
end)
...
do
local skinState = {equipped={}, favorites={}, hooked=false, originals={}, mods={}, viewingProfile=nil, saveFile="onyx_skins_v2.json"}
...
local function instancePreloadCoreModules()
    ...
    task.spawn(function() pcall(function() instanceSafeRequire(modulesFolder:WaitForChild("InputLibrary", 4)) end) end)
end
...
local function onyxLoad(url, name)
    ...
    if not ok then warn("[Onyx] HttpGet failed for " .. name .. ": " .. tostring(src)) return nil end
    if not src or #src == 0 then warn("[Onyx] Empty response for " .. name) return nil end
    local fn, err = loadstring(src)
    if not fn then warn("[Onyx] loadstring failed for " .. name .. ": " .. tostring(err)) return nil end
    local res, ret = pcall(fn)
    if not res then warn("[Onyx] Runtime error in " .. name .. ": " .. tostring(ret)) return nil end
    return ret
end
...
-- Old Kicia-style gun sound disabler system disabled (replaced with custom mute/unmute)
...
-- Old: task.defer(setupShootDetection) -- disabled, using custom mute/unmute
...
updaimbot()
-- setupShootDetection() -- disabled, using custom mute/unmute
...
local function getHitEffectFont(style)
    -- Only use font changer's custom font for Fortnite style
    if style == "Fortnite" then
        local font = getgenv().InstanceUIFont
        -- Ensure it's a Font object, not EnumItem
...
local function fortniteffect(adornee, color, damageText)
    ...
    -- Create fragments (one per character)
    local fragments = {}
    ...
    for i = 1, #damageText do
        ...
        -- Random shatter velocity for later
    ...
    -- Phase 1: Float up for aliveDuration
    ...
    floatConn = RunService.RenderStepped:Connect(function()
        ...
        if elapsed > aliveDuration then
            ...
            shatterConn = RunService.RenderStepped:Connect(function()
                ...
                local easeProgress = 1 - (1 - progress) * (1 - progress) -- ease out quad
...
-- RootDesync: Heartbeat sets fake CFrame (server sees it), RenderStep restores original (client sees normal)
local RootDesync = {}
...
-- ShootLock: rate-limits firing based on weapon fire rate
local ShootLock = {}
...
-- DefensiveCFrame: computes orientation based on item behavior (kiciahook's getDefensiveCFrame)
...
-- recordBackstab: sets hitbox window and cooldown (kiciahook timing)
...
-- encodeCameraRotation: encodes a CFrame orientation to 2-byte format (kiciahook)
...
-- Server-visible camera randomizer: modifies the ENCODED rotation sent to
-- the server only. Your client view is never touched.
...
Utility.EncodeCameraRotation = function(self, vec2, ...)
    -- Camera randomize: continuous vertical rotation (pitch axis = vec2.X) so the
    -- side of your head spins up/down like a rolling wheel. No jitter.
    if settings.camerarandomizeenabled and (settings.camrandomx > 0 or settings.camrandomy > 0 or settings.camrandomz > 0) then
        ...
        -- 360*4 subdivision spin accumulators, one per axis
        ...
        -- Map each axis into -180..180 and fold onto the 2-axis encoder:
        -- pitch (vec2.X) = X spinner + Z spinner, yaw (vec2.Y) = Y spinner
...
-- rivalss style: find SetControls remote at startup
local ReplicatedStorage = game:GetService("ReplicatedStorage")
...
-- No FireServer method hook (causes "not a valid member" errors).
-- The 1s reapply loop below keeps the spoof applied (rivalss style).
...
-- Auto reapply every 1s (rivalss style)
task.spawn(function()
    while true do
        task.wait(1)
...
ThemeManager:SetFolder('onyx')
ThemeManager.DefaultTheme = 'UE'
SaveManager:SetFolder('onyx/rivals')
...
task.spawn(function()
    ...
    pcall(function()
        if isfile("onyx/autoload.txt") and readfile("onyx/autoload.txt") == "true" then
...
do
    ...
    do
        ...
        do
            ...
            task.spawn(function()
                task.wait(1.5)
                local okCL, cl = pcall(instanceSafeRequire, ReplicatedStorage:WaitForChild("Modules", 20):WaitForChild("CosmeticLibrary", 20), 10)
                local okIL, il = pcall(instanceSafeRequire, ReplicatedStorage.Modules:WaitForChild("ItemLibrary", 20), 10)
                local okRC, rc = pcall(instanceSafeRequire, ReplicatedStorage.Modules:WaitForChild("ReplicatedClass", 20), 10)
                ...
                local okCI, ci = pcall(instanceSafeRequire, Modules:WaitForChild("ClientReplicatedClasses", 15):WaitForChild("ClientFighter", 15):WaitForChild("ClientItem", 15), 10)
                local okCVM, cvm
                if okCI and ci then
                    okCVM, cvm = pcall(instanceSafeRequire, Modules.ClientReplicatedClasses.ClientFighter.ClientItem:WaitForChild("ClientViewModel", 15), 10)
                end
...
do
    ...
    ConfigManageGroup:AddToggle('AutoLoadToggle', {
        Text = 'auto load config',        Default = false,
        Callback = function(val)
            getgenv().InstanceAutoloadEnabled = val
            pcall(function()
                if val then
                    if not isfolder("onyx") then makefolder("onyx") end
                    ...
                    writefile("onyx/autoload.txt", "true")
                    ...
                    writeLoader("autoexec/onyxloader.lua", "onyx/onyxnew.txt")
                    pcall(function()
                        ...
                        if ok and content and #content > 100 then
                            writefile("onyx/onyxnew.txt", content)
                        end
                    end)
                else
                    writefile("onyx/autoload.txt", "false")
                    pcall(delfile, "autoexec/onyxloader.lua")
                end
            end)
        end
    })
